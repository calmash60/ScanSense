...
export default function WelcomePage() {
    ...
    useEffect(() => {
        ...
        // This prevents the redirect for unauthenticated users.
        if (!loading && user) {
            router.push('/scanner');
        }
    }, [user, loading, router]);

    // Show a loading state while checking for user auth,
    // but only if we are not sure yet if a user is logged in.
    if (loading) {
        return (
    ...
    // If user is loaded and is not authenticated, show the welcome page.
    if (!user) {
        return (
    ...
    // If the user is logged in, they have already been redirected,
    // but we can return null or a loading indicator just in case.
    return null;
}
